...
class Mongo_db
{
	 ...
	 */
	public function GridFS($name='fs'){
		$this->GridFS=$this->db->getGridFS($name);
		return $this->GridFS;
	}
	 ...
	 */
	public function count($collection = '', $isClear = true)
	{
		if (empty($collection)) {
		...
		public function findAndModify($collection = '',$update=array(),$options=array()){
			$documents = $this->db->{$collection}->findAndModify($this->wheres,$update,$this->selects,$options);
			$this->_clear();
			return $documents;
		}
	...
	public function insert($collection = '', $insert = array())
	{
		...
		try {
			$this->db->{$collection}->insert($insert, array($this->query_safety));
			if (isset($insert['_id'])) {
				return $insert['_id'];
			} else {
				return false;
	...
	private function connect()
	{
		...
		try {
			$this->connection = new MongoClient($this->connection_string, $options);
						//设置节点读取
						$this->connection->setReadPreference(MongoClient::RP_NEAREST, array());
			$this->db = $this->connection->{$this->dbname};
			return $this;
		} catch (MongoConnectionException $e) {
			if ($this->CI->config->item('mongo_supress_connect_error')) {
				# show_error('Unable to connect to MongoDB', 500);
				show_error('网络波动, 请刷新后重试或者联系管理员, 错误代码MDB-10083021', 500);
			} else {
				# show_error("Unable to connect to MongoDB: {$e->getMessage()}", 500);
				show_error("网络波动, 请刷新后重试或者联系管理员, 错误代码MDB-10083021: {$e->getMessage()}", 500);
			}
		}
	}
	 ...
	 */
	private function connection_string($options)
	{
		$this->CI->config->load($this->config_file);
		if (isset($options['isTest'])) {
			$this->dbname = trim($this->CI->config->item($options['test_db']));
		} else {
		...
		$this->replicaSet = trim($this->CI->config->item('replicaSet'));
		$dbhostflag = (bool) $this->CI->config->item('host_db_flag');
		$hosttypeflag = $this->CI->config->item('host_type_flag');
		$mongohosts = $this->CI->config->item('mongo_hosts');
		...
		if ($hosttypeflag == 1) {
		...
		} elseif ($hosttypeflag == 2) {
			$host_str = '';
			foreach ($mongohosts as  $host) {
				if ($host_str) {
					if (isset($this->port) && !empty($this->port)) {
						$host_str .= ",{$host['mongo_host']}:{$host['mongo_port']}";
					} else {
						$host_str .= ",{$host['mongo_host']}";
					}
				} else {
					if (isset($this->port) && !empty($this->port)) {
						$host_str .= "{$host['mongo_host']}:{$host['mongo_port']}";
			...
			if ($dbhostflag === true) {
				$host_str = trim($host_str).'/'.$this->dbname;
			} else {
				$host_str = trim($host_str);
			}
			$this->connection_string = $connection_string.$host_str;
		}
	}
	...
	/**
	 * 参数：$collection 表名
	 * $keys 关键字 需要加$符号来获取表中的字段值
	 * $groupKey 结果中的key
	 * $initail 生成结果的方法
	 * $sort 排序方法.
	 */
	public function aggregate($collection, $keys, $groupKey, $initial, $sort)
	{
		$cond = array();
		...
		$result = $this->db->{$collection}->aggregate($cond);
		$this->_clear();

		return $result;
	}

	public function aggregate_simple($collection, $pipeline, $options = null)
	{
		if(!empty($options)){
			$result = $this->db->{$collection}->aggregate($pipeline, $options);
		}
		$result = $this->db->{$collection}->aggregate($pipeline);
		$this->_clear();

		return $result;
	}

	public function group($collection, $keys, $initial, $reduce)
	{
		$result = $this->db->{$collection}->group($keys, $initial, $reduce, array('condition' => $this->wheres));
		$this->_clear();

		return $result;
	}

	public function like_or($likes)
	{
		if (count($likes) < 0) {
			return $this;
		}
		foreach ($likes as $key => $like) {
			...
			$value = $like['value'];
			$flags = $like['flag'];
			$enable_start_wildcard = isset($like['start']) ? $like['start'] : false;
			$enable_end_wildcard = isset($like['end']) ? $like['end'] : false;
			$field = (string) trim($field);
			$value = (string) trim($value);
			$value = quotemeta($value);
	...
	public function distinct($key, $condition, $collection){
		$result = $this->db->{$collection}->distinct($key, $condition);
		$this->_clear();
		return $result;
	}

	public function setReadPreference($mode=1){
		...
		$this->connection->setReadPreference($type, array());
		$this->db = $this->connection->{$this->dbname};
	}

	public function test($data){
		$connection = new MongoClient($this->connection_string, array());
